  ...
  return (
    <tr key={location.id + 1}>
      <th
        scope='row'
        onClick={() =>
          // row.edit && setRow({ ...row, edit: false, name: row.oldName })
          console.log("Click")
        }
      >
        {props.index + 1}
      ...
      <td>
        {row.edit ? (
          ...
          <span
            onDoubleClick={() =>
              setRow({ ...row, edit: true, oldName: row.name })
            }
          >
            {row.name}
          </span>
      ...
      <td onDoubleClick={() => deleteL(location.id)}>
...
export default connect(mapStateToProps, mapDispatchToProps)(LocationTableRow);
